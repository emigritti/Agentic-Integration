...
- **Remote:** https://github.com/emigritti/Agentic-Integration
- **Purpose:** Integrating agentic AI systems
...
| Frontend | React                                   |
| Backend  | Python                                  |
| LLM      | To be decided during project development |
| Deploy   | Docker                                  |
...
### Docker (from repo root — run from WSL)
All traffic enters via the nginx reverse proxy. Internally tutto su porta 80; esternamente
mappato a **8080** (porta 80 su Windows è occupata da IIS/WAS in questo ambiente dev).
```bash
docker compose up --build          # build all images and start → http://localhost:8080
docker compose up -d --build       # same, detached
docker compose down                # stop and remove containers
docker compose logs -f proxy       # stream proxy logs
...
| `/api/` | `backend:8000` *(reserved — not yet active)* |

### Backend (`backend/`) — not yet scaffolded
```bash
...
```
docs/
  adr/                    # Architecture Decision Records (template: ADR-000-template.md)
  test-plan/              # Test plans (template: TEST-PLAN-000-template.md)
  unit-test-review/       # Unit test review checklist (UNIT-TEST-REVIEW-CHECKLIST.md)
```
...
## Data Usage Boundary (Accenture Compliance)
...
## Responsible AI (Accenture Principles)
...
## Default Working Method (Mandatory)
...
Skipping steps is NOT allowed if not explicitated by user
...
## Feature Planning (Feature Dev Plugin)
...
## Unit Testing — First‑Class Quality Gate
...
## Superpowers Plugin Usage (Mandatory)
...
## feature-dev Plugin Usage (Mandatory)
...
## Security & Secure Coding (Mandatory)
